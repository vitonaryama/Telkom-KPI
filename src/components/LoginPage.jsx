    ...
    <div className="min-h-screen w-full flex bg-white">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-slate-950">
        <svg className="absolute inset-0 w-full h-full opacity-40" viewBox="0 0 800 900" preserveAspectRatio="xMidYMid slice">
          ...
          {Array.from({ length: 18 }).map((_, i) => (
            <line key={`h${i}`} x1="0" y1={i * 50} x2="800" y2={i * 50} stroke="url(#gridFade)" strokeWidth="1" />
          ))}
          {Array.from({ length: 16 }).map((_, i) => (
            <line key={`v${i}`} x1={i * 50} y1="0" x2={i * 50} y2="900" stroke="url(#gridFade)" strokeWidth="1" />
          ))}
        </svg>
        {/* Signal tower motif */}
      ...
      <div className="flex-1 flex items-center justify-center px-6 sm:px-10 py-12">
        <div className="w-full max-w-sm">
          ...
          <p className="text-sm text-gray-500 mb-8">Pantau KPI IOAN Anda dari sini.</p>
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-5">
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <a href="#" className="text-red-600 font-medium hover:underline">Lupa Kata Sandi?</a>
            </div>
            ...
            <p className="text-center text-sm text-gray-500">
              Belum punya akun? <a href="#" className="text-red-600 font-medium hover:underline">Daftar</a>
            </p>
          </form>
        </div>
      </div>
